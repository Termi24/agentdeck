...
4. **`apps/web`** — Next.js 15 + React 19 + Tailwind 4 + shadcn/ui + `dockview-react`. Three primary routes:
   - **`/`** — the hub. Global KPI bar (active connections, running agents/tools, live activity), filter toolbar (search + status tabs + project dropdown + live-only toggle + grid/list view), session cards with per-session micro-stats + last channel message preview + expandable sub-agents. Header carries a **Findings** badge linking to `/internal/findings`, highlighted orange when there are any open `error|critical` self-bugs. Refreshes every 8 s (REST poll only, no Socket.IO on the hub yet).
   - **`/projects/[id]`** — sessions filtered to one project, **plus** a "Teams in {projectId}" section (FB-03, v0.0.9): one row per session = one team, click opens a side-sheet listing the orchestrator + sub-agents with full skill/prompt (read-only). Auto-redirects to `/` when `default` is the only project so the implicit bucket doesn't duplicate the hub.
   - **`/sessions/[id]`** — single-session dashboard. Sticky header with title/status/live dot/tokens/stream indicator + overflow menu. KPI strip with **5 cards** (sub-agents / tool calls / **planning** w/ breakdown / channel / tests). Main pair: **AgentTree** (tree with info icon → side-sheet "Agent detail" with full skill/prompt + DMs ; FB-01 client-side `useAgentStuckStatus` hook adds a stuck badge on agents silent ≥ 3 min) + **ActivityFeed** (unified timeline of channel + tool calls + docs + tests + agent lifecycle + watchdog warnings/interventions, with filter tabs + auto-scroll + "N new" jump-to-latest, virtualized above 500 events). Row 3 tabs: Agents & context (skill tiles) / Planning (Gantt + Calendar + Progress) / DMs / Docs / Tests / Channel history. Sticky footer with `UserInputBar` for human injections via `await_user_input`. The legacy `RunningTools` side panel was removed in v0.0.9 (FB-04) — its info is covered by the KPI strip + tool_call entries in the feed.
   - **`/sessions/[id]/dockview`** — classic tiling-panels view, preserved as a power-user alternative.
   - **`/internal/findings`** — admin page for the FB-10 self-bug-tracker. Filters by status × severity, table triable, click row → side-sheet with message/stack/context/fingerprint, actions {triage, mark fixed, wontfix, purge}.
